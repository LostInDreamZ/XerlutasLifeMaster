...
_result set[2,[_temp] call DB_fnc_numberSafe];

_loadout = [(_result select 3)] call DB_fnc_mresToArray;
if(typeName _loadout == "STRING") then {_loadout = call compile format["%1",_loadout]};
_result set[3,_loadout];

_loadout = [(_result select 4)] call DB_fnc_mresToArray;
if(typeName _loadout == "STRING") then {_loadout = call compile format["%1",_loadout]};
_result set[4,_loadout];

_loadout = [(_result select 5)] call DB_fnc_mresToArray;
if(typeName _loadout == "STRING") then {_loadout = call compile format["%1",_loadout]};
_result set[5,_loadout];

_loadout = [(_result select 6)] call DB_fnc_mresToArray;
if(typeName _loadout == "STRING") then {_loadout = call compile format["%1",_loadout]};
_result set[6,_loadout];

_loadout = [(_result select 7)] call DB_fnc_mresToArray;
if(typeName _loadout == "STRING") then {_loadout = call compile format["%1",_loadout]};
